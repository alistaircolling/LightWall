	...
	void incrFallDelay() {
		...
		void matrixEQ()
		{
		  ...
		  for (int i = 0; i < matrix.cols(); i++)
		  {
		    ...
		    int tmpPos = constrain( (int)((matrixVal * MATRIX_ROWS) * tmpDrawFactor), 0, MATRIX_ROWS-1);
		    ...
		    matrix.drawLine(i, MATRIX_ROWS-1-tmpPos, i, MATRIX_ROWS-1, r, g, b);
		    ...
		    if (eqPos[i]>300) {
		      if (MATRIX_ROWS-1-eqPos[i]-1 > 0) {
		        tmpBack = java.awt.Color.HSBtoRGB(tColor, (float).4, colorEQBackBrt*2);
		      ...
		      if (MATRIX_ROWS-1-eqPos[i]-2 > 0) {
		        tmpBack = Color.HSBtoRGB(tColor, (float).7, colorEQBackBrt*2);
		        tmpBackColor = new Color(tmpBack);
			    rb = tmpBackColor.getRed(); //(int)red(tmpBackColor);
			    gb = tmpBackColor.getGreen(); //(int)green(tmpBackColor);
			    bb = tmpBackColor.getBlue(); //(int)blue(tmpBackColor);
...
//					ProcessingAppLauncher procLaunch = new ProcessingAppLauncher();
	...
	public MatrixEQApp() {
		...
		txtSongName.setText("c:/data/DemoRap.mp3");
